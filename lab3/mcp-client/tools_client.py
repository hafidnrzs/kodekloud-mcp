            ...
            async with ClientSession(read, write) as client:
                ...
                try:
                    ...
                    if prompt_result.messages and len(prompt_result.messages) > 0:
                        ...
                        if hasattr(prompt_result.messages[0].content, "text"):
                            print(
                                f"   {prompt_result.messages[0].content.text[:300]}..."
                            )
                        else:
                            print(
                                f"   {str(prompt_result.messages[0].content)[:300]}..."
                            )
                    print()
                except Exception as e:
                ...
                try:
                    disruption_result = await client.get_prompt(
                        "handle_disruption",
                        {"original_flight": "FL123", "reason": "weather delay"},
                    )
                    if (
                        ...
                        if hasattr(disruption_result.messages[0].content, "text"):
                            ...
                            print(
                                f"   {str(disruption_result.messages[0].content)[:300]}..."
                            )
                    print()
                except Exception as e:
