...
async def handle_sampling(
    ...
    if user_messages:
        ...
        prompt = (
            user_messages[-1].content.text
            if hasattr(user_messages[-1].content, "text")
            else str(user_messages[-1].content)
        )
...
Exploring worlds across the seas."""
        elif "explain" in prompt.lower() and (
            "flight" in prompt.lower() or "travel" in prompt.lower()
        ):
...
5. Arrive at the airport with plenty of time"""
            ...
            response = (
                f"Here's a helpful response about flight booking and travel: {prompt}"
            )
    ...
    return CreateMessageResult(
        role="assistant",
        content=TextContent(
            type="text", text="I couldn't generate a response for that request."
        ),
        model="simulated-llm",
        stopReason="endTurn",
    )
    ...
    try:
        async with streamablehttp_client("http://localhost:8000/mcp/") as (
            ...
            async with ClientSession(
                read, write, sampling_callback=handle_sampling
            ) as client:
                ...
                if not sampling_tools:
                    ...
                    print(
                        "     This is normal - the basic flight server doesn't have LLM tools"
                    )
                print()
